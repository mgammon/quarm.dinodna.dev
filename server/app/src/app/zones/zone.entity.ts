...
export interface Zone {
  ...
  reducedspawntimers: number;
  trivial_loot_code: number;
...
// TODO:  toss era field onto zones table, it'll be empty for some things like instances, but whatever.
// Pull from in-memory on API rather than this giant static list.
...
export function getZoneIds() {
  let zoneIds: { [zoneId: string]: string } = {};
  allZones.forEach(
    (zone) => (zoneIds = { ...zoneIds, ...{ [zone.id]: zone.name } })
  );
  return zoneIds;
}
