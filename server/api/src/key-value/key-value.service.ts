...
export class KeyValueService {
  ...
  async autoUpdate() {
    ...
    const currentDumpNameKeyValue = await this.getKeyValue(CURRENT_DUMP_NAME_KEY);
    // If we couldn't check the repo, or we could and we're already using the most recent dump, no need to update.
    if (!mostRecentDump || (currentDumpNameKeyValue && mostRecentDump.name === currentDumpNameKeyValue.value)) {
    ...
    try {
      ...
      await new DatabaseUpdater().initializeQuarmData(true, mostRecentDump.download_url);
      ...
      this.feedbackService.sendGeneral(`[${process.env.NODE_ENV}] Error auto-updating the DB ${ex}`);
    } finally {
